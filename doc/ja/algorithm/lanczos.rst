...
HPhi++ では、\ :math:`T_{n}`\ の対角化にlapackのルーチン
...
.. [#] 高速化のために、\In HPhi++ ではハミルトニアンの対角成分を表すベクトル1本と,スピン :math:`z` 成分 :math:`S_{z}` 保存, 粒子数保存の場合はその状態を指定するベクトル1本を余計に確保しています。いずれのベクトルの大きさもHilbert空間の次元です。 
